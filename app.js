...
const BIT_DURATION    = 0.1;   // seconds per bit (100ms)
const PREAMBLE_DURATION = 0.3; // seconds preamble is held
const FADE_TIME       = 0.005; // seconds, click-avoidance ramp

const FFT_SIZE          = 4096;
const ENERGY_THRESHOLD  = 140;   // 0-255 scale from getByteFrequencyData
const CONFIRM_FRAMES    = 5;     // consecutive frames needed to confirm preamble
...
enableMicBtn.addEventListener('click', async () => {
  try {
    // IMPORTANT: disable browser audio processing. By default Chrome/Android
    // apply echo cancellation, noise suppression, and auto-gain control to
    // the mic stream — these are tuned for voice and will often filter out
    // or heavily attenuate near-ultrasonic tones like ours (17-18.5kHz),
    // making the receiver "deaf" even though the tone is genuinely playing.
...
function playBitSequence(bits) {
  return new Promise((resolve) => {
    ...
    gainNode.gain.linearRampToValueAtTime(0.4, startTime + FADE_TIME);
    ...
    gainNode.gain.setValueAtTime(0.4, t - FADE_TIME);
